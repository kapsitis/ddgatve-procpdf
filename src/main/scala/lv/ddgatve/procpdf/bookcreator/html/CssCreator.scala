...
// http://stackoverflow.com/questions/30905145/mathml-to-image-like-jeuclid-that-works-on-gae
// http://developers.itextpdf.com/examples/xml-worker/html-images
object CssCreator {
...
"""
     ...
     val str = "<html><head><title>Test PDF</title></head><body><div>" + 
    ...
    val writer = PdfWriter.getInstance(document,
      new FileOutputStream("resources/lab01.pdf"))
      
    document.open()

    // CSS
    val cssResolver = new StyleAttrCSSResolver()
    val cssFile = XMLWorkerHelper.getCSS(new ByteArrayInputStream(myCSS.getBytes()))
    cssResolver.addCss(cssFile)
    // HTML
    val htmlContext = new HtmlPipelineContext(null)
    ...
    val p = new XMLParser(worker, Charset.forName("UTF-8"))
    p.parse(new FileInputStream("resources/lab02.html"))
